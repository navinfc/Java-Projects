...
public class PlayAreaController {
    ...
    private Pair<Double, Double> movement(double translateX, double translateY, int rolling) {
        ...
        else
        {
            ...
            {
                if(moveY == -450){
                    return (new Pair<>(translateX, translateY));
                }
                moveX = -1*(moveX-50);
                moveY -= 50;
            }
    ...
    void checkWin(double x, double y) throws IOException {
        if(x == 50 && y == -450){
            ...
            if(turn == 1) winAlert.setContentText("Player 1 won");
            else winAlert.setContentText("Player 2 won");
